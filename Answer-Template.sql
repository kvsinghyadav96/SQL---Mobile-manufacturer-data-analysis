...
SELECT TOP 1
COUNTRY, State, Manufacturer_Name,
...
SELECT COUNT(*) AS TRAN_COUNT FROM (
					  SELECT DISTINCT
					  IDModel, ZipCode, State
					  FROM DIM_LOCATION AS T1
...
RIGHT JOIN (
			SELECT
			 IDCustomer,
			AVG(TotalPrice) AS AVG_AMT, DATEPART(YEAR, DATE) AS [YEAR]
...
--SELECT * FROM FACT_TRANSACTIONS
 ...
 ( SELECT TOP 10 IDCustomer,
   ...
   select * from TOP_10_CUS
  YEAR_SPEND AS
  (SELECT TOP 10 IDCUSTOMER,
   AVG(TOTALPRICE),
   AVG(QUANTITY)
   FROM FACT_TRANSACTIONS
   WHERE IDCustomer IN (SELECT IDCUSTOMER FROM TOP_10_CUS)
   )

  SELECT *,
  LAG(TOTALPRICE,1) OVER( ORDER BY TOTALPRICE) AS PREV,
  (TOTALPRICE - LAG(TOTALPRICE,1) OVER( ORDER BY TOTALPRICE) ) / LAG(TOTALPRICE,1) OVER( ORDER BY TOTALPRICE) * 100 AS PERCENT_SPEND 
  FROM YEAR_SPEND
